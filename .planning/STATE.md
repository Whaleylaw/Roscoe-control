...
status: completed
stopped_at: "Plan 14-07 complete. Three pure-logic lib modules under src/lib/ — runner-preamble.ts (generatePreamble for tool-agnostic /recipe/PREAMBLE.md text), runner-worktree.ts (seedMcDir/readMcTaskJson/writeMcTaskJson/buildPriorAttemptsEntry for .mc/ seeding), runner-docker.ts (buildDockerRunArgs/stageRecipe/writeEnvFile/cleanupEnvFile/slugify for docker-run argv + env-file + recipe-stage). 36 Vitest cases across 5 files (7+10+9+6+4) replacing 30 it.todo scaffold stubs. Plan 14-08b consumes via inline re-declaration with pointer-comment back to src/lib/ as source of truth. Commits 8cb9a1b / 6af3ff6 / 8480a76. Preamble forward-references POST /api/runner/checkpoint (Phase 15) AND POST /api/runner/tasks/\\$MC_TASK_ID/submit (RAUTH-06 allowlist-safe, NOT legacy PUT /api/tasks/:id). CONTAINER-01 invariant enforced: no --env flag on argv carries MC_API_TOKEN — secrets flow via --env-file only."
last_updated: "2026-04-20T18:23:52Z"
last_activity: "2026-04-20 — Plan 14-07 executed. Task 1 commit 8cb9a1b (runner-preamble.ts + 7 tests — first-attempt 30-50 non-blank lines, resume 35-55, deterministic). Task 2 commit 6af3ff6 (runner-worktree.ts + 10 tests — task.json always rewritten on resume, progress.md/checkpoints.jsonl preserved, .gitignore literally '*\\n', 0600 perms on task.json). Task 3 commit 8480a76 (runner-docker.ts + 19 tests across 3 files — labels + mounts + --add-host + --env-file, image LAST in argv, slugify 'My Ref #01'->'my-ref-01'). Decisions: Plan 14-08b uses inline re-declaration (not tsx/bundle) per locked strategy; stageRecipe writes PREAMBLE.md AFTER deep-copy so runner owns /recipe/PREAMBLE.md; newline-in-value sanitised to space defensively."
progress:
  total_phases: 14
  completed_phases: 10
  total_plans: 52
...
Last session: 2026-04-20T18:23:52Z
Stopped at: Plan 14-07 complete. Three pure-logic lib modules landed under src/lib/ — runner-preamble.ts (generatePreamble tool-agnostic Markdown body for /recipe/PREAMBLE.md with checkpoint+submit HTTP skeleton forward-reference), runner-worktree.ts (seedMcDir/readMcTaskJson/writeMcTaskJson/buildPriorAttemptsEntry; task.json always rewritten, progress.md+checkpoints.jsonl preserved on resume, .gitignore literally '*\\n', 0600 perms), runner-docker.ts (buildDockerRunArgs with locked argv order + CONTAINER-01 secrets-only-via-env-file invariant, stageRecipe async deep-copy + PREAMBLE.md-last, writeEnvFile/cleanupEnvFile mode-0600 with newline sanitation, slugify). 36 Vitest cases across 5 files all pass; lint 0 errors on plan-owned files; plan-owned typecheck clean. Commits 8cb9a1b (preamble) / 6af3ff6 (worktree) / 8480a76 (docker). Plan 14-08b will inline-duplicate these helpers into scripts/mc-runner.mjs with a pointer comment back to src/lib/ as source of truth.
Resume file: .planning/phases/14-runner-container-v1-2/14-07-SUMMARY.md
