...
- [ ] **Gap C / M1 / P0.3 (scheduler): Lane-aware default auto-routing** — `autoRouteInboxTasks()` in `src/lib/task-dispatch.ts` routes globally from `status='inbox'` by role-keyword affinity, ignoring lifecycle readiness. Prefer inbox tasks linked to active `in_progress` plans (`gsd_plan_id`) and scoped project lanes; fall back to unscoped legacy inbox only when no lane-scoped work is eligible. Emit route-reason metadata (`auto_route_lane_scoped` / `auto_route_legacy_fallback`). Preserve recipe fast-path.
- [ ] **Gap D / M2 / P0.3 (blocker): Unified blocker transition contract** — legacy dispatch is retry/fail-oriented; recipe path has owner-wait semantics. Add structured `in_progress → awaiting_owner` transition on legacy path with `blocker_reason`, `blocker_kind`, `resume_hint`, preserve retry/resume metadata; add deterministic owner-resume. Emit common events for both paths. Files: `src/lib/task-dispatch.ts`, `src/lib/task-checkpoints.ts`, `src/app/api/tasks/[id]/route.ts` (if status rules need extension).
