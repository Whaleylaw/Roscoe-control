...
**No signed HIPAA.** Records request cannot proceed. Expected in `client/authorizations.md` with linked evidence to a HIPAA or medical authorization shadow. If missing, the caller needs to finish Phase 0 document collection before this skill can run.

**Missing provider fax.** Provider ledger/contact has no `fax` value and the master card has no `fax` either. Either look it up and add it to the master card (`Contacts/Medical/<slug>.md`), fall back to email/mail, or queue for manual send.
...
Every error, retry, and workaround gets an activity entry (`cases/<slug>/activity/<YYYY-MM-DD-HHMM>-correspondence.md` for send attempts, `-phone.md` for call-based recovery). The activity log is the durable case audit trail.
