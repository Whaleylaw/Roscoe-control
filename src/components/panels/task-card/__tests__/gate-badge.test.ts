...
function renderWithIntl(ui: React.ReactNode) {
  return render(
    React.createElement(
      ...
      { messages: messages as any, locale: 'en' },
      ui
    )
  )
}
...
describe('GateBadge (GSD-25)', () => {
  it('gate_required=0 → renders nothing', () => {
    const { container } = renderWithIntl(
      React.createElement(GateBadge, { task: { gate_required: 0, gate_status: 'not_required' } })
    )
    expect(container).toBeEmptyDOMElement()
  })

  it('gate_required=undefined → renders nothing', () => {
    const { container } = renderWithIntl(React.createElement(GateBadge, { task: {} }))
    expect(container).toBeEmptyDOMElement()
  })

  it('gate_required=1, gate_status="approved" → green badge with "✓ Approved"', () => {
    renderWithIntl(
      React.createElement(GateBadge, { task: { gate_required: 1, gate_status: 'approved' } })
    )
  ...
  it('gate_required=1, gate_status="pending" → amber "🔒 Approval required"', () => {
    renderWithIntl(
      React.createElement(GateBadge, { task: { gate_required: 1, gate_status: 'pending' } })
    )
  ...
  it('gate_required=1, gate_status="rejected" → amber "🔒 Approval required"', () => {
    renderWithIntl(
      React.createElement(GateBadge, { task: { gate_required: 1, gate_status: 'rejected' } })
    )
  ...
  it('gate_required=1, gate_status="not_required" → amber "🔒 Approval required" (any non-approved state)', () => {
    renderWithIntl(
      React.createElement(GateBadge, { task: { gate_required: 1, gate_status: 'not_required' } })
    )
    expect(screen.getByText('🔒 Approval required')).toBeInTheDocument()
  })
})
