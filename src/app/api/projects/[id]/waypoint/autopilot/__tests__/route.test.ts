...
function malformedJsonReq(path: string, rawBody: string) {
  return new NextRequest(`http://localhost${path}`, {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: rawBody,
...
describe('POST /api/projects/:id/waypoint/autopilot', () => {
  ...
  it('returns consistent parse envelope for malformed JSON body', async () => {
    ...
    const { POST } = await loadRoute()
    const res = await POST(malformedJsonReq(`/api/projects/${projectId}/waypoint/autopilot`, '{'), {
      params: Promise.resolve({ id: String(projectId) }),
    })
  ...
  it('returns normalized validation details for invalid request body', async () => {
    ...
    const { POST } = await loadRoute()
    const res = await POST(req(`/api/projects/${projectId}/waypoint/autopilot`, { max_iterations: 0 }), {
      params: Promise.resolve({ id: String(projectId) }),
    })
