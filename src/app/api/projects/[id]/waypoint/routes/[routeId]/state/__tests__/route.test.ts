...
`,
  ...
  const scope = db
    .prepare(
      `
    SELECT gpl.id AS plan_id, gp.id AS phase_id, gm.id AS milestone_id, gm.workstream_id
    FROM gsd_plans gpl
...
async function loadStateRoute() {
  return import('@/app/api/projects/[id]/waypoint/routes/[routeId]/state/route')
}
...
describe('POST /api/projects/:id/waypoint/routes/:routeId/state', () => {
  it('rejects viewer role', async () => {
    authRole = 'viewer'
    const projectId = seedProject({ gsdEnabled: 1 })

    const { POST } = await loadStateRoute()
    const res = await POST(postReq(`/api/projects/${projectId}/waypoint/routes/1/state`, { action: 'pause' }), {
  ...
  it('pauses and resumes a route', async () => {
    ...
    const paused = await POST(postReq(`/api/projects/${projectId}/waypoint/routes/${routeId}/state`, { action: 'pause' }), {
      params: Promise.resolve({ id: String(projectId), routeId: String(routeId) }),
    })
    expect(paused.status).toBe(200)
    const pausedBody = await paused.json()
    expect(pausedBody.action).toBe('pause_route')
    expect(pausedBody.route.status).toBe('blocked')

    const resumed = await POST(postReq(`/api/projects/${projectId}/waypoint/routes/${routeId}/state`, { action: 'resume' }), {
      params: Promise.resolve({ id: String(projectId), routeId: String(routeId) }),
    })
    expect(resumed.status).toBe(200)
    const resumedBody = await resumed.json()
    expect(resumedBody.action).toBe('resume_route')
    expect(resumedBody.route.status).toBe('active')
  })
})
